...
import {tuple, isPoint, isVector, point, vector, 
  ...
  magnitude, normalize, dot, cross, reflect} from '../src/tuples.js';
...
test('tuples', function (t) {
  ...
  t.test('Reflecting a vector approaching at 45 degrees', t => {
    const v = vector(1, -1, 0);
    const n = vector(0, 1, 0);
    const r = reflect(v, n);
    t.equal(r, vector(1, 1, 0));
  });

  t.test('Reflecting a vector off a slanted surface', t => {
    const v = vector(0, -1, 0);
    const n = vector(Math.sqrt(2) / 2, Math.sqrt(2) / 2, 0);
    const r = reflect(v, n);
    t.ok(M.equal([r], [vector(1, 0, 0)]));
  });
